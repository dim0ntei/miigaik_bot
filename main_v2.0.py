...
async def menu(message: Message):
    ...
    await message.answer("Основное меню:", keyboard=keyboards.KEYBOARD_MAIN_MENU)
...
async def who_for_info(message: Message):
    ...
    await message.answer("Для начала, выбери факультет:", keyboard=keyboards.KEYBOARD_FACULTY_WHO_FOR)


@bot.on.private_message(text='Назад', state=WhoForStates.who_for_faculty)
async def back_to_menu(message: Message):
    await message.answer("Возвращаемся в основное меню...", keyboard=keyboards.KEYBOARD_MAIN_MENU)
...
async def who_for_year(message: Message):
    ...
    await message.answer("В каком учебном году?", keyboard=keyboards.KEYBOARD_YEAR)
...
async def who_for_sem(message: Message):
    ...
    await message.answer("В каком семестре?", keyboard=keyboards.KEYBOARD_SEM)
...
async def your_var(message: Message):
    ...
    await message.answer("Какой порядковый номер в списке группы? Введи цифрами:", keyboard=keyboards.EMPTY_KEYBOARD)
...
async def result(message: Message):
    ...
    if ctx.get('who_for_confirmation') == 'Да':
        try:
            ...
            await message.answer("Возвращаемся в меню...", keyboard=keyboards.KEYBOARD_MAIN_MENU)
            await bot.state_dispenser.delete(message.peer_id)
            ...
            await message.answer("Ошибка. Возможно, вы задали некорректные условия поиска. "
                                 "Отправить отчет разработчику?", keyboard=keyboards.KEYBOARD_CONFIRMATION)
    elif ctx.get('who_for_confirmation') == 'Нет':
        await message.answer("Начни заново:)", keyboard=keyboards.KEYBOARD_MAIN_MENU)
        await bot.state_dispenser.delete(message.peer_id)
...
async def who_for_error(message: Message):
    ...
    if ctx.get('who_for_error_confirmation') == 'Да':
        ...
        await message.answer("Отчет отправлен администратору. Возвращаемся в меню...", keyboard=keyboards.KEYBOARD_MAIN_MENU)
        await bot.state_dispenser.delete(message.peer_id)
    ...
    await message.answer('Выберите «да» или «нет»', keyboard=keyboards.KEYBOARD_CONFIRMATION)
...
async def executors_list(message: Message):
    await message.answer('В связи с тем, что исполнителей пока что мало, они выводятся одним списком. '
                         'В будущем будет фильтрация по факультету.', keyboard=keyboards.KEYBOARD_WORKS_MENU)
    ...
    await message.answer(f"{final_exec_list}", keyboard=keyboards.KEYBOARD_WORKS_MENU)
...
async def ready_works_list(message: Message):
    ...
    await message.answer(f"{final_work_list}", keyboard=keyboards.KEYBOARD_WORKS_MENU)
...
async def garant_request(message: Message):
    ...
    await message.answer("Настоятельно рекомендую ознакомиться с правилами работы сервиса «Гарант» здесь:\n\n"
                         ...
                         "\n\nПродолжить оформление запроса?", keyboard=keyboards.KEYBOARD_CONFIRMATION)
...
async def garant_request_customer(message: Message):
    ...
    if ctx.get('garant_request_confirmation') == 'Да':
        ...
        await message.answer("Возвращаемся в меню...", keyboard=keyboards.KEYBOARD_WORKS_MENU)
        await bot.state_dispenser.set(message.peer_id, WorksMenuStates.works_menu)
...
async def garant_request_executer(message: Message):
    ...
    await message.answer("Отправьте ссылку на аккаунт ВК исполнителя:", keyboard=keyboards.EMPTY_KEYBOARD)
...
async def send_garant_request(message: Message):
    ...
    if ctx.get('send_garant_request') == 'Да':
        ...
        await message.answer("Запрос отправлен на модерацию. В случае одобрения вам придет сообщение в этот чат. "
                             "Возвращаемся в меню...", keyboard=keyboards.KEYBOARD_WORKS_MENU)
...
async def additional_menu(message: Message):
    ...
    await message.answer("Дополнительное меню", keyboard=keyboards.KEYBOARD_ADDITIONAL_MENU)
...
async def registration(message: Message):
    ...
    await message.answer('Выберите, кем хотите стать. Исполнители выполняют работы на заказ, продавцы - продают свои '
                         'старые готовые работы.', keyboard=keyboards.KEYBOARD_REGISTRATION)


@bot.on.private_message(state=RegistrationStates.main, text='Назад')
async def reg_back(message: Message):
    await message.answer('Возвращаемся...', keyboard=keyboards.KEYBOARD_ADDITIONAL_MENU)
    await bot.state_dispenser.set(message.peer_id, AdditionalMenuStates.main)
...
async def executor_reg_conf(message: Message):
    ...
    await message.answer('Заполнить анкету?', keyboard=keyboards.KEYBOARD_CONFIRMATION)
...
async def executor_reg_faculty(message: Message):
    ...
    if ctx.get('executor_reg_conf') == 'Нет':
        await message.answer('Возвращаемся...', keyboard=keyboards.KEYBOARD_REGISTRATION)
        await bot.state_dispenser.get(RegistrationStates.main)
    elif ctx.get('executor_reg_conf') == 'Да':
        await message.answer('С какого вы факультета?', keyboard=keyboards.KEYBOARD_FACULTY)
...
async def executor_reg_about(message: Message):
    ...
    await message.answer('Напишите, за какие предметы вы готовы браться, какие типы работ предпочитаете (РГР, '
                         'курсовые, домашние работы и т.д. и т.п.)', keyboard=keyboards.EMPTY_KEYBOARD)
...
async def executor_send_conf(message: Message):
    ...
    await message.answer('Отправить заявку? После одобрения вы будете отображаться в списке исполнителей в течении 3-х '
                         'месяцев.\n\nОбращаем внимание, что спам карается баном.', keyboard=keyboards.KEYBOARD_CONFIRMATION)
...
async def executor_reg_end(message: Message):
    ...
    if ctx.get('executor_reg_conf') == 'Да':
        ...
        await message.answer("Анкета отправлена. Возвращаемся...", keyboard=keyboards.KEYBOARD_MAIN_MENU)
        await bot.state_dispenser.delete(message.peer_id)
        await bot.state_dispenser.get(RegistrationStates.main)
    elif ctx.get('executor_reg_conf') == 'Нет':
        await message.answer('Возвращаемся...', keyboard=keyboards.KEYBOARD_MAIN_MENU)
        await bot.state_dispenser.delete(message.peer_id)
        await bot.state_dispenser.get(WorksMenuStates.works_menu)
...
async def phone_book_menu(message: Message):
    ...
    await message.answer("Выберите раздел", keyboard=keyboards.KEYBOARD_PHONE_BOOK)
...
async def phone_book(message: Message):
    ...
    if ctx.get('phone_group') == 'Деканаты':
        ...
        await message.answer("Если вы точно уверены, что какой-либо из контактов неверен, используйте функцию "
                             "предложить исправление.", keyboard=keyboards.KEYBOARD_PHONE_BOOK)
...
async def do_not_answer(message: Message):
    ...
    if ctx.get('do_not_answer') == '!продолжить':
        await message.answer('Возобновляю работу', keyboard=keyboards.KEYBOARD_MAIN_MENU)
        await bot.state_dispenser.delete(message.peer_id)
...
async def unknown_command(message: Message):
    await message.answer("Я вас не понимаю. Воспользуйтесь меню.", keyboard=keyboards.KEYBOARD_MAIN_MENU)
